...
describe('TimeRange', () => {
  ...
  test('endAt が startAt + duration になる', () => {
    ...
    const range = new TimeRange(startAt, duration);
      ...
      expect(range.endAt.value).toBe('2026-01-04T03:00:00.000+09:00');
  });
  ...
  test('contains() start は含む / end は含まない', () => {
    const range = new TimeRange(
      ...
      expect(range.contains(new DateTime('2026-01-04T01:59:59.999+09:00'))).toBeTruthy();
      expect(range.contains(new DateTime('2026-01-04T02:00:00.000+09:00'))).toBeFalsy();
  });
